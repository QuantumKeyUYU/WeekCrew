...
export default function CirclePage() {
  ...
  // 🔹 старт круга при заходе — ТОЛЬКО если в сторе ещё нет круга
  useEffect(() => {
    if (circle || loadingCircle) return;
    ...
    const startFreshCircle = async () => {
      ...
      try {
        ...
        // сообщения при первом заходе берём из ответа joinCircle
    ...
    return () => {
      cancelled = true;
      // ВАЖНО: НЕ выходим из круга на unmount —
      // иначе при перезагрузке круг всегда будет новым.
    };
  }, [circle, loadingCircle, setCircle, setMessages, setQuotaFromApi]);

  // начальная загрузка сообщений с бэка (если он что-то хранит)
  useEffect(() => {
    if (!circle || notMember) {
