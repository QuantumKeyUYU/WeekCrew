...
export default function CirclePage() {
  ...
  const messages = useDemoCircleStore((state) => state.messages);
  const sendMessage = useDemoCircleStore((state) => state.sendMessage);
  const joinedAt = useDemoCircleStore((state) => state.joinedAt);
  ...
  const interestMeta = useMemo(() => (currentInterestKey ? INTERESTS_MAP[currentInterestKey] : null), [currentInterestKey]);
  ...
  if (!currentInterestKey || !interestMeta) {
    return (
      ...
      <section className={panelClass}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{interestMeta.title}</h1>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{interestMeta.description}</p>
          </div>
          <div className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-200">
            <span>До конца недели: {remainingDays} дн.</span>
            <span>В команде сейчас: {DEFAULT_MEMBERS} человек</span>
          </div>
        </div>
      </section>
        ...
        <div ref={listRef} className="mt-4 max-h-[55vh] space-y-3 overflow-y-auto pr-2">
          {messages.map((msg) => (
            <div key={msg.id} className={clsx('flex', msg.from === 'me' ? 'justify-end' : 'justify-start')}>
              ...
              >
                <p>{msg.text}</p>
                <span className={clsx('mt-2 block text-[11px]', msg.from === 'me' ? 'text-white/80' : 'text-slate-500')}>
