...
export default function ExplorePage() {
  ...
  const startMatching = useWeekcrewStore((state) => state.startMatching);
  const panelClass =
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
        ...
        {INTERESTS.map((interest) => (
          <button
            ...
            className={clsx(
              ...
              selected === interest.id && status === 'matching'
              ...
              status === 'matching' && selected !== interest.id ? 'cursor-wait opacity-60' : 'cursor-pointer'
            )}
      ...
      <div className="rounded-3xl border border-slate-200/80 bg-white/95 p-4 text-sm text-slate-600 shadow-[0_10px_26px_rgba(15,23,42,0.05)] dark:border-white/10 dark:bg-slate-900/60 dark:text-slate-200" aria-live="polite">
        ...
        {status === 'matching' && <p>{t('explore_status_loading')}</p>}
      </div>
